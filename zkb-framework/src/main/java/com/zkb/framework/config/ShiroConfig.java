...
import com.zkb.framework.shiro.realm.UserRealm;
import com.zkb.framework.shiro.session.OnlineSessionDAO;
import com.zkb.framework.shiro.session.OnlineSessionFactory;
import com.zkb.framework.shiro.web.CustomShiroFilterFactoryBean;
import com.zkb.framework.shiro.web.filter.LogoutFilter;
import com.zkb.framework.shiro.web.filter.captcha.CaptchaValidateFilter;
import com.zkb.framework.shiro.web.filter.kickout.KnockoutSessionFilter;
import com.zkb.framework.shiro.web.filter.online.OnlineSessionFilter;
...
public class ShiroConfig {
   ...
   /* public CookieRememberMeManager rememberMeManager() {
     ...
     * 同一个用户多设备登录限制
     */
    public KnockoutSessionFilter kickoutSessionFilter() {
        KnockoutSessionFilter knockoutSessionFilter = new KnockoutSessionFilter();
        knockoutSessionFilter.setCacheManager(getEhCacheManager());
        knockoutSessionFilter.setSessionManager(sessionManager());
        // 同一个用户最大的会话数，默认-1无限制；比如2的意思是同一个用户允许最多同时两个人登录
        knockoutSessionFilter.setMaxSession(maxSession);
        // 是否踢出后来登录的，默认是false；即后者登录的用户踢出前者登录的用户；踢出顺序
        knockoutSessionFilter.setKnockoutAfter(kickoutAfter);
        // 被踢出后重定向到的地址；
        knockoutSessionFilter.setKnockoutUrl("/login?kickout=1");
        return knockoutSessionFilter;
    }
